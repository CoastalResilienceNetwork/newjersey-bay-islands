...
function eventListeners(){
	...
	$('#innerContent .checkboxes').on('click',function(c){
		if (c.target.checked){
			...
			let ben = $(`#${c.target.id}`).parent().parent().parent().find(".checkboxes-wrap")[0].id.split("-").pop();
			app[ben] = "";
			layerDefs();
			$.each($('#' + c.target.id).parent().parent().next().find('input'),function(i,v){
				$(v).attr('disabled', true)		
	...
	document.querySelector("#close-selected-island").addEventListener('click', (() => {
		closeSelectedIsland();
	}))
...
function layerDefs(){
	app.obj.exp = [app.prcnt_mhw, app.Erosn_Acre, app.brng_cap, app.Ditch_feet, app.mid_tram, app.bird_util, app.Ownership, app.ww_dist_mi, app.sav_summry, app.tmc_hm_pct, app.tmc_lm_pct, app.tmc_mf_pct, app.tmc_ow_pct, app.tmc_ph_pct, app.tmc_pp_pct, app.tmc_up_pct, app.t_range_ft, app.sc_avg_ft, app.omwm_acres];
	var exp = "";
	var cnt = 0;
