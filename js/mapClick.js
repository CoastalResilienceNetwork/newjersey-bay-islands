...
function createClickInfo(response){
    ...
    document.getElementById("selected-island").innerHTML = `
   		...
   		Island Acreage: <b>${acres}</b><br>
		Edge Erosion 1977-2015 (acres): <b>${eroAcres}</b><br>
		Marsh Tidal Ditching (feet): <b>${ditchFeet}</b><br>
		Source Delineation: <b>${a.Source}</b><br>
		Marsh Substrate Bearing Capacity: <b>${brng_cap}</b><br>
		MIDTRAM-Score: <b>${mid_tram}</b><br>
		Island Ownership Combine: <b>${a.Ownership}</b><br>
		Tidal Range (feet): <b>${t_range_ft}</b><br>
		Closest Waterway Type: <b>${a.ww_type}</b><br>
		Closest Waterway (miles): <b>${ww_dist_mi}</b><br>
		Acreage Above MHW: <b>${mhw_above}</b><br>
		Percent Above MHW: <b>${prcnt_mhw}%</b><br>
		...
		Closest Years SAV: <b>${a.sav_sum_yr}</b><br>
		...
		Average shoreline change 1977 - 2015 (feet): <b>${sc_avg_ft}</b><br>
