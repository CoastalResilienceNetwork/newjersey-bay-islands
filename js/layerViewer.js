function buildLayerViewer() {
  if (layerViewer.show) {
    ...
    require(['esri/request'], function (esriRequest) {
      esriRequest(layerViewer.url + '/layers?f=pjson', {
        responseType: 'json',
      }).then(function (response) {
        // The requested data
        let layerJson = response.data;
        layerJson.layers.unshift({
          name: 'Nautical Charts (NOAA ENC)',
          id: 99,
        });
        ...
        if (layerViewer.location == 'sidebar') {
          document.querySelector('#innerContent').insertAdjacentHTML(
            'beforeend',
            `
		      			<section>
        ...
        if (layerViewer.location == 'overmap') {
          ...
          document
            .querySelector('#overmap-toggle-button')
            .addEventListener('click', (tb) => {
              if (tb.target.classList.contains('esri-icon-collapse')) {
                document.getElementById('overmap-toggle-button').innerHTML = `
		      					<span aria-hidden="true" class="esri-collapse__icon esri-icon-collection"></span>
		      					<span class="esri-icon-font-fallback-text">Expand</span>
		      				`;
                document.getElementById('overmap-toggle-button').title =
                  'Supporting Layers';
                document.getElementById(
                  'layer-viewer-overmap-wrap'
                ).style.display = 'none';
              }
          ...
          if (layerViewer.expand) {
            document.getElementById('overmap-toggle-button').innerHTML = `
	      						...
	      						<span class="esri-icon-font-fallback-text">Collapse</span>
		      				`;
            document.getElementById('layer-viewer-overmap-wrap').style.display =
              'block';
        ...
        layerJson.layers.forEach((l) => {
          ...
          if (layerViewer.skipLayers.indexOf(l.id) == -1) {
            ...
            if (l.type == 'Group Layer' && !l.parentLayer) {
              //topGroup.push(l)
              document.querySelector('#layer-viewer-wrap').insertAdjacentHTML(
                'beforeend',
                `
			               		<div>
								...
								<div class="flex1 layer-viewer-row" style="position:relative;">
						    		<label class="form-component" for="sup_cb_-_${l.id}">
						      		...
						      		<div class="info-dropdown">
												...
												<div id="closeInfo" style="color:red; font-weight:bold">X &nbsp;</div>
            ...
            if (l.type == 'Group Layer' && l.parentLayer) {
              if (document.querySelector(`#layerGroup-${l.parentLayer.id}`)) {
                document
                  .querySelector(`#layerGroup-${l.parentLayer.id}`)
                  .insertAdjacentHTML(
                    'beforeend',
                    `
        ...
        app.supportingLayers.when(function () {
          ...
          document
            .querySelectorAll('#layer-viewer-wrap .sup_cb')
            .forEach((cb) =>
              cb.addEventListener('click', () => {
                if (cb.value < 90) {
                  let sublayer = app.supportingLayers.findSublayerById(
                    parseInt(cb.value)
                  );
          ...
          document.querySelectorAll('.group-title').forEach((gr) =>
            gr.addEventListener('click', () => {
              if (
                gr.parentNode.querySelector('.fa-caret-down').style.display ==
                'none'
              ) {
                gr.parentNode.querySelector('.group-wrap').style.display =
                  'block';
                gr.parentNode.querySelector('.fa-caret-down').style.display =
                  'inline-block';
                gr.parentNode.querySelector('.fa-caret-right').style.display =
                  'none';
              } else {
                gr.parentNode.querySelector('.group-wrap').style.display =
                  'none';
          ...
          document.querySelectorAll('.layer-viewer-info').forEach((info) =>
            info.addEventListener('click', () => {
              let id = info.id.split('-').pop();
              if (id < 90) {
                esriRequest(app.url + '/' + id + '?f=pjson', {
                  responseType: 'json',
                }).then(function (li) {
                  ...
                  document.querySelector(`#opacity-title-${id}`).innerHTML =
                    li.data.name;
                  let h =
                    document.querySelector(`#opacity-title-${id}`)
                      .offsetHeight + 11;
                  document.getElementById(`desc-wrap-${id}`).style.marginTop =
                    h + 'px';
                });
              } else {
                if (id == 99) {
              ...
              info.parentNode.querySelector('.info-dropdown').style.display =
                'block';
            })
          );
          // hide info popup when mouse leaves
          document.querySelectorAll('.info-dropdown').forEach((row) =>
